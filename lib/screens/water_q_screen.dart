...
class WaterQScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (scenarioData.aiTheme == 'muddy') {
      weatherIcon = Icons.flash_on; // 오염 상태일 때 번개
      weatherColor = Colors.grey;
    } else if (scenarioData.aiTheme == 'cloudy' || scenarioData.aiTheme == 'salty') {
      weatherIcon = Icons.cloud;    // 흐림 상태일 때 구름
      weatherColor = Colors.blueGrey;
    } else {
      weatherIcon = Icons.wb_sunny; // 맑음 상태일 때 해
    ...
    return Scaffold(
          ...
          Builder(
            builder: (context) => GestureDetector(
              onTap: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => const WeatherTestScreen()),
                );
              },
              onLongPress: () {
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildAiDeviceBtn(context, "세탁기", Icons.local_laundry_service, 'washer'),
          const SizedBox(height: 10),
          _buildAiDeviceBtn(context, "식기세척기", Icons.restaurant, 'dishwasher'), // 아이콘 변경 (기본 restaurant)
          const SizedBox(height: 10),
          _buildAiDeviceBtn(context, "정수기", Icons.water_drop, 'purifier'),
          const SizedBox(height: 10),
          _buildAiDeviceBtn(context, "건조기", Icons.air, 'dryer'),
          const SizedBox(height: 10),
          _buildAiDeviceBtn(context, "스타일러", Icons.checkroom, 'styler'),
        ],
      ),
    );
  }
